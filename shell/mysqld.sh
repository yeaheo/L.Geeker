...
# install boost
tar zxf /opt/boost_1_59_0.tar.gz -C /usr/local

# install cmake
tar zxf /opt/cmake-*.tar.gz -C /usr/src
cd /usr/src/cmake-*
...
tar zxf /opt/mysql-*.tar.gz -C /usr/src/
cd /usr/src/mysql-*/
echo -e "\033[33m 正在配置Mysql... \033[0m"
cmake  -DCMAKE_INSTALL_PREFIX=/usr/local/mysql   -DSYSCONFDIR=/etc  -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci  -DWITH_EXTRA_CHARSETS=all -DWITH_BOOST=/usr/local/boost_1_59_0/ &> /dev/null
echo -e "\033[33m 配置完成！ \033[0m"
